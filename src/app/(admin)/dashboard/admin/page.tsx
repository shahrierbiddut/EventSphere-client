...
import { BarChart2, BookOpen, DollarSign, Ticket, Users, ArrowUpRight } from "lucide-react";
import { AdminStatsCard } from "@/components/admin/AdminStatsCard";
import { AdminBadge, AdminSkeletonRow } from "@/components/admin/AdminUI";
import { BookingsOverviewChart, BookingStatusPieChart } from "@/components/admin/AdminCharts";
  ...
  return (
    ...
    >
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div>
            ...
            <p className="text-sm text-indigo-200/80 font-medium mt-2 max-w-xl">
              Welcome back to your command center. Monitor user growth, event capacities, revenue aggregates, and platform statistics in real time.
            </p>
          </div>
          <div className="flex-shrink-0">
            <div className="px-5 py-3.5 bg-white/10 border border-white/10 backdrop-blur-md rounded-2xl flex flex-col items-center md:items-end">
              <span className="text-[10px] text-indigo-300 font-extrabold uppercase tracking-widest">Local Server Time</span>
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <div className="xl:col-span-2 bg-white rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow p-6 md:p-8 flex flex-col justify-between">
          <div className="flex items-center justify-between border-b border-slate-50 pb-4 mb-6">
            <div>
              <h2 className="text-lg font-bold text-slate-800 tracking-tight">Bookings & Revenue Overview</h2>
              <p className="text-xs text-slate-400 font-medium mt-0.5">Aggregate performance over the past six months</p>
            </div>
            <div className="flex items-center gap-1.5 bg-slate-50 rounded-xl p-1 text-xs border border-slate-100">
              <span className="px-3 py-1 bg-white rounded-lg font-bold text-slate-700 shadow-sm border border-slate-100">6 Months</span>
            </div>
          </div>
        ...
        <div className="bg-white rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow p-6 md:p-8 flex flex-col justify-between">
          <div className="border-b border-slate-50 pb-4 mb-6">
            <h2 className="text-lg font-bold text-slate-800 tracking-tight">Booking Status</h2>
            <p className="text-xs text-slate-400 font-medium mt-0.5">Summary of ticket states</p>
          </div>
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-1 xl:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden flex flex-col">
          <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 bg-slate-50/40">
            <div>
              <h2 className="text-base font-bold text-slate-800 tracking-tight">Recent Booking Transactions</h2>
              <p className="text-[11px] text-slate-400 font-medium">Real-time payment logs</p>
            </div>
            <a href="/dashboard/admin/bookings" className="inline-flex items-center gap-1 text-xs text-indigo-600 font-bold hover:text-indigo-700 group transition-colors">
              View sales ledger <ArrowUpRight size={14} className="group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform" />
            </a>
          </div>
          <div className="overflow-x-auto flex-1">
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-slate-50">
                {loading ? (
                  Array.from({ length: 4 }).map((_, i) => <AdminSkeletonRow key={i} cols={4} />)
                ) : stats?.recentBookings.map((booking) => (
                  <tr key={booking._id} className="hover:bg-slate-50/40 transition-colors">
                    <td className="px-6 py-4">
                      <p className="font-semibold text-slate-800 text-xs">{booking.user.name}</p>
                      <p className="text-slate-400 text-[10px] font-medium">{booking.user.email}</p>
                    </td>
                    <td className="px-6 py-4 text-xs font-semibold text-slate-600 max-w-[150px] truncate">{booking.event.title}</td>
                    <td className="px-6 py-4">
        ...
        <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden flex flex-col">
          <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 bg-slate-50/40">
            <div>
              <h2 className="text-base font-bold text-slate-800 tracking-tight">Latest Registrants</h2>
              <p className="text-[11px] text-slate-400 font-medium">New member notifications</p>
            </div>
            <a href="/dashboard/admin/users" className="inline-flex items-center gap-1 text-xs text-indigo-600 font-bold hover:text-indigo-700 group transition-colors">
              Manage accounts <ArrowUpRight size={14} className="group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform" />
            </a>
          </div>
          <div className="divide-y divide-slate-50 flex-1">
            {loading ? (
              ...
              return (
                <div key={user._id} className="flex items-center gap-4 px-6 py-4 hover:bg-slate-50/40 transition-colors">
