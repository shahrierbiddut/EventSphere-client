      ...
      <div className="bg-gradient-to-br from-indigo-900 via-purple-900 to-indigo-800 text-white py-16 px-4">
        <div className="container mx-auto max-w-4xl text-center">
          ...
          <p className="text-lg text-indigo-100/80 max-w-2xl mx-auto">
            Discover the best events happening around you. Filter by category, date, location, and price.
          </p>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 md:px-6 py-8">
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="w-full lg:w-72 shrink-0">
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 sticky top-20 space-y-6">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input placeholder="City or area" className="pl-9 bg-gray-50 border-gray-200 focus:bg-white" />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input type="date" className="pl-9 bg-gray-50 border-gray-200 focus:bg-white" />
                </div>
              </div>
                  ...
                  {[
                    ...
                    <label key={value} className="flex items-center gap-2.5 cursor-pointer group">
                      <input
                        ...
                        onChange={() => setPriceFilter(value as "all" | "free" | "paid")}
                        className="text-primary focus:ring-primary"
                      />
                      <span className={`text-sm transition-colors ${priceFilter === value ? "text-primary font-semibold" : "text-gray-600 group-hover:text-gray-900"}`}>
                        {label}
                      </span>
                    </label>
                  ))}
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-col gap-1">
                  <button
                    className={`text-left text-sm px-3 py-2.5 rounded-xl transition-all flex items-center gap-2 ${!selectedCategory ? "bg-primary text-white shadow-md shadow-primary/20 font-semibold" : "hover:bg-gray-50 text-gray-600 border border-transparent hover:border-gray-200"}`}
                    onClick={() => setSelectedCategory(null)}
                  >
                    <span>🌐</span> All Categories
                    <span className={`ml-auto text-xs px-1.5 py-0.5 rounded-full font-medium ${!selectedCategory ? "bg-white/20 text-white" : "bg-gray-100 text-gray-500"}`}>
                      {events.length}
                    </span>
                  </button>
                  {categories.map((cat) => {
                    ...
                    return (
                      <button
                        key={cat.id}
                        className={`text-left text-sm px-3 py-2.5 rounded-xl transition-all flex items-center gap-2 ${selectedCategory === cat.name ? "bg-primary text-white shadow-md shadow-primary/20 font-semibold" : "hover:bg-gray-50 text-gray-600 border border-transparent hover:border-gray-200"}`}
                        onClick={() => setSelectedCategory(cat.name)}
                      >
                        <span>{categoryIcons[cat.name] || "📌"}</span>
                        {cat.name}
                        <span className={`ml-auto text-xs px-1.5 py-0.5 rounded-full font-medium ${selectedCategory === cat.name ? "bg-white/20 text-white" : "bg-gray-100 text-gray-500"}`}>
                          {count}
                        </span>
                      </button>
                    );
          ...
          <div className="flex-1 space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
              <div>
                <p className="text-sm font-medium text-gray-900">
                  {hasActiveFilters ? (
                    ...
                    <><span className="text-primary font-bold">{filteredEvents.length}</span> events available</>
                  )}
                </p>
                {hasActiveFilters && (
              ...
              <div className="flex items-center gap-3 w-full sm:w-auto">
                <select
                  className="text-sm border border-gray-200 rounded-xl py-2 pl-3 pr-8 w-full sm:w-auto bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-primary/30"
                  value={sortBy}
                ...
                <div className="flex items-center border border-gray-200 rounded-xl overflow-hidden shrink-0 bg-white">
                  <button
                    className={`p-2.5 transition-colors ${viewMode === "grid" ? "bg-primary/10 text-primary" : "hover:bg-gray-50 text-gray-400"}`}
                  ...
                  <button
                    className={`p-2.5 transition-colors ${viewMode === "list" ? "bg-primary/10 text-primary" : "hover:bg-gray-50 text-gray-400"}`}
              ...
              <div className="flex flex-wrap gap-2">
                {selectedCategory && (
                  <span className="inline-flex items-center gap-1.5 bg-primary/10 text-primary text-xs font-semibold px-3 py-1.5 rounded-full border border-primary/20">
                    ...
                    <button onClick={() => setSelectedCategory(null)} className="hover:text-primary/60">
                      <X className="h-3 w-3" />
                    </button>
                  </span>
                )}
                {priceFilter !== "all" && (
                  <span className="inline-flex items-center gap-1.5 bg-green-100 text-green-700 text-xs font-semibold px-3 py-1.5 rounded-full border border-green-200">
                    ...
                    <button onClick={() => setPriceFilter("all")} className="hover:text-green-500">
                      <X className="h-3 w-3" />
                    </button>
                  </span>
                )}
            ...
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-24 px-4 text-center border-2 border-dashed border-gray-200 rounded-2xl bg-white">
                <div className="w-20 h-20 bg-gray-100 rounded-full flex items-center justify-center text-3xl mb-5">
                  🔍
                </div>
                <h3 className="text-xl font-bold text-gray-900 mb-2">No events found</h3>
                <p className="text-gray-500 max-w-sm mb-6">
                  We couldn't find events matching your filters. Try adjusting your search.
                </p>
            ...
            {!isLoading && totalPages > 1 && (
              <div className="flex justify-center pt-4">
                <div className="flex items-center gap-2 bg-white border border-gray-100 rounded-2xl shadow-sm px-4 py-3">
                  <button
                    onClick={() => handlePageChange(currentPage - 1)}
                    disabled={currentPage === 1}
                    className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-medium border border-gray-200 text-gray-600 hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed transition-all"
                  >
                  ...
                  <div className="flex items-center gap-1">
                    {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => {
                      ...
                      if (showEllipsisBefore || showEllipsisAfter) {
                        return <span key={`ellipsis-${page}`} className="px-2 text-gray-400 text-sm">...</span>;
                      }
    ...
    <React.Suspense fallback={
      <div className="min-h-screen bg-gray-50 flex items-center justify-center py-24">
        <div className="text-center space-y-4">
          ...
          <p className="text-gray-500 font-medium">Loading events explore page...</p>
        </div>
      </div>
    }>
