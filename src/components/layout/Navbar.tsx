      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ease-in-out ${
        isScrolled 
          ? user 
            ...
            : "bg-white/95 backdrop-blur-md shadow-sm border-b border-gray-200/60 py-3" 
          : user 
    ...
    >
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between gap-6">
        ...
        <Link href="/" className="flex items-center gap-2.5 shrink-0 group">
          ...
          <span className={`font-heading font-semibold text-xl tracking-tight ${user ? "text-white" : "text-gray-900"}`}>EventSphere</span>
        </Link>
        ...
        <div className="hidden lg:flex items-center justify-end gap-3 shrink-0">
          ...
          {user ? (
            ...
            <>
              ...
              {/* Favorites */}
              <Link href="/dashboard/favorites" className="p-2.5 text-slate-400 hover:text-white hover:bg-slate-800 rounded-xl transition-colors relative block">
                <Heart size={20} />
              </Link>

              {/* Notifications */}
              <Link href="/dashboard/notifications" className="p-2.5 text-slate-400 hover:text-white hover:bg-slate-800 rounded-xl transition-colors relative block">
                <Bell size={20} />
              ...
              <div className="relative profile-dropdown-container ml-2">
                ...
                >
                  <div className="relative">
                    {user.photoUrl ? (
                      <img src={user.photoUrl} alt={user.name} className="h-9 w-9 rounded-full object-cover border border-slate-700" />
                    ) : (
                  ...
                  <span className="text-sm font-medium text-slate-200">Hi, {user.name?.split(" ")[0] || "User"}</span>
                </button>
                ...
                {isProfileOpen && (
                  <div className="absolute right-0 mt-3 w-64 bg-[#0F172A] rounded-2xl shadow-xl shadow-slate-900/20 border border-slate-800 overflow-hidden py-2 transform opacity-100 scale-100 transition-all origin-top-right">
                    ...
                    <div className="px-4 py-3 border-b border-slate-800/80 flex items-center gap-3">
                      {user.photoUrl ? (
                        <img src={user.photoUrl} alt={user.name} className="h-10 w-10 rounded-full object-cover border border-slate-700" />
                      ) : (
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-slate-400 truncate">View your profile</p>
                      </div>
                    </div>

                    <div className="px-2 py-2 space-y-0.5">
                      <Link href="/dashboard" onClick={() => setIsProfileOpen(false)} className="flex items-center gap-3 px-3 py-2 text-sm text-slate-300 hover:text-white hover:bg-slate-800 rounded-xl transition-colors">
                        <LayoutDashboard size={16} /> Dashboard
                      </Link>
                      <Link href="/dashboard/bookings" onClick={() => setIsProfileOpen(false)} className="flex items-center gap-3 px-3 py-2 text-sm text-slate-300 hover:text-white hover:bg-slate-800 rounded-xl transition-colors">
                        <CalendarDays size={16} /> My Bookings
                      </Link>
                      <Link href="/dashboard/profile" onClick={() => setIsProfileOpen(false)} className="flex items-center gap-3 px-3 py-2 text-sm text-slate-300 hover:text-white hover:bg-slate-800 rounded-xl transition-colors">
                        <UserIcon size={16} /> My Profile
                      </Link>
                      <Link href="/dashboard/settings" onClick={() => setIsProfileOpen(false)} className="flex items-center gap-3 px-3 py-2 text-sm text-slate-300 hover:text-white hover:bg-slate-800 rounded-xl transition-colors">
                        <Settings size={16} /> Settings
                      </Link>
                      <Link href="/faq" onClick={() => setIsProfileOpen(false)} className="flex items-center gap-3 px-3 py-2 text-sm text-slate-300 hover:text-white hover:bg-slate-800 rounded-xl transition-colors">
            ...
            /* Guest Right Actions */
            <>
              <button className="p-2.5 text-gray-500 hover:text-gray-900 hover:bg-gray-100 rounded-xl transition-colors">
                <Search size={20} />
              </button>
              <button className="p-2.5 text-gray-500 hover:text-gray-900 hover:bg-gray-100 rounded-xl transition-colors">
                <Moon size={20} />
              ...
              <Link href="/login">
                <Button variant="outline" className="rounded-[14px] px-5 py-2 h-auto text-sm font-semibold border-gray-200 text-gray-700 hover:bg-gray-50">
                  Login
                </Button>
              </Link>
              <Link href="/register">
        ...
        {/* Mobile Menu Toggle */}
        <button
          className={`lg:hidden p-2 rounded-xl transition-colors ${user ? "text-slate-300 hover:bg-slate-800" : "text-gray-600 hover:bg-gray-100"}`}
      ...
      {/* Mobile Drawer */}
      {isMobileMenuOpen && (
        <div className={`lg:hidden absolute top-full left-0 right-0 border-b shadow-xl overflow-y-auto max-h-[calc(100vh-80px)] ${user ? "bg-[#0F172A] border-slate-800 shadow-slate-900/50" : "bg-white border-gray-200 shadow-black/5"}`}>
            ...
            {user ? (
              ...
              <>
                <div className="flex items-center gap-4 p-4 bg-slate-800/50 rounded-2xl border border-slate-700/50">
                  <div className="relative">
                    {user.photoUrl ? (
                      <img src={user.photoUrl} alt={user.name} className="h-12 w-12 rounded-full object-cover border border-slate-700" />
                    ) : (
                ...
                <div className="flex gap-2">
                  <div className="flex-1 bg-slate-800/50 rounded-2xl p-3 flex flex-col items-center justify-center gap-1 border border-slate-700/50">
                    ...
                    <span className="text-xs font-medium text-slate-300">Favorites</span>
                  </div>
                  <div className="flex-1 bg-slate-800/50 rounded-2xl p-3 flex flex-col items-center justify-center gap-1 border border-slate-700/50 relative">
                    ...
                    <span className="absolute top-3 right-8 flex h-4 w-4 items-center justify-center rounded-full bg-indigo-600 text-[9px] font-bold text-white border-2 border-[#0F172A]">3</span>
                ...
                <nav className="space-y-1">
                  ...
                  <div className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3 px-2">Account</div>
                  
                  <Link href="/dashboard" className="flex items-center gap-3 px-4 py-3 rounded-xl text-[15px] font-medium text-slate-300 hover:bg-slate-800 hover:text-white transition-colors">
                    <LayoutDashboard size={18} className="text-slate-400" /> Dashboard
                  </Link>
                  <Link href="/dashboard/settings" className="flex items-center gap-3 px-4 py-3 rounded-xl text-[15px] font-medium text-slate-300 hover:bg-slate-800 hover:text-white transition-colors">
              ...
              <>
                <div className="relative">
                  ...
                  <input 
                    type="text" 
                    placeholder="Search events..."
                    className="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-base focus:outline-none focus:border-indigo-500"
                  />
                </div>
                ...
                <nav className="space-y-1">
                  {navLinks.map((link) => (
                    <Link
                      key={link.name}
                      href={link.href}
                      className="block px-4 py-3 rounded-xl text-[15px] font-medium text-gray-700 hover:bg-gray-50 hover:text-indigo-600 transition-colors"
                    >
                      {link.name}
                    </Link>
                  ))}
                ...
                <div className="border-t border-gray-100 pt-6 space-y-3">
                  <Link href="/login" className="block w-full">
                    <Button variant="outline" className="w-full rounded-xl py-6 text-base font-semibold border-gray-200">
                      Login
                    </Button>
                  </Link>
